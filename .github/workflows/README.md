...
This GitHub Actions workflow automatically builds Wine for both x86_64 and aarch64 (ARM64EC) architectures.
...
Builds Wine for both architectures in parallel:
...
5. **Artifact Packaging**
   ...
   - Generates two `profile.json` files with build metadata:
     - `profile.json` - Wine type for GameNative
     - `profile-wine.json` - Wine type for Winlator for CMOD & Ludashi
   - Creates two output files:
     - **Wine WCP**: Contains Wine-type profile for GameNative (wcp format)
...
- **Push** to `main`, `master`, or `wine-11.x` branch
- **Pull requests** to `main`, `master`, or `wine-11.x` branch
...
- Included in GitHub releases (for pushes to main/master/wine-11.x)

### Artifact Names

**Output Files**

**Wine Type (for GameNative):**
- `wine-11.8-1-x86_64.wcp` - x86_64 Wine wcp
- `wine-11.8-1-arm64ec.wcp` - arm64ec Wine wcp

**Wine Type (for Winlator CMOD & Ludashi):**
- `wine-11.8-1-x86_64.wcp.xz` - x86_64 Wine wcp.xz
- `wine-11.8-1-arm64ec.wcp.xz` - arm64ec Wine wcp.xz
